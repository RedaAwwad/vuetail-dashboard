...
<template>
    <aside
        ...
        <PerfectScrollbar class="h-[calc(100vh-(75px+65px))] py-6 px-3 font-medium">
            <ul class="space-y-2">
                <li v-for="(route, index) in dashboardRoutes" :key="index">
                    <RouterLink exact :to="{ name: route.name }" @click="openDropdownLinks(route)"
                        ...
                        <span v-if="route.count"
                            class="inline-flex items-center justify-center h-2.5 p-3 ms-3 text-sm font-medium text-blue-800 bg-blue-100 rounded-full dark:bg-primary dark:text-gray-100">{{ route.count }}</span>
                        <span v-if="route.children.length" class="inline-block transition-shadow"
        ...
        <p
            ...
            <a href="https://www.linkedin.com/in/redaawwad/" target="_blank" class="text-primary hover:text-blue-500">
                Need Help?
            </a>
        </p>
    </aside>
